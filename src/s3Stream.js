...
// Cache for the S3 stream instance
let _s3Stream = null;
...
function getStream(config = {}) {
    // If a force refresh wasn't requested and we already have a stream and no new config was provided, return the cached instance
    if (_s3Stream && Object.keys(config).length === 0 && !config.forceRefresh) {
        return _s3Stream;
    }

    // Get AWS credentials from global constants or environment variables
    ...
    // Default configuration with more environment variable fallbacks
    const defaultConfig = {
        ...
        name_format: process.env.S3_LOG_NAME_FORMAT || `%Y-%m-%d-%H-%M-${APP_TYPE}-error.log`,
        rotate_every: process.env.S3_LOG_ROTATE_EVERY || '1h',
        max_file_size: parseInt(process.env.S3_LOG_MAX_FILE_SIZE || (1024 * 1024 * 5)), // 5 MB
        upload_every: parseInt(process.env.S3_LOG_UPLOAD_EVERY || (3 * 60 * 60 * 1000)), // 3 hours
    ...
    if (!mergedConfig.access_key_id || !mergedConfig.secret_access_key) {
        console.warn('S3StreamLogger: Missing AWS credentials. S3 logging disabled.');
        return null;
    }

    try {
        // Create and cache the S3 stream
...
/**
 * Resets the cached S3 stream instance
 */
